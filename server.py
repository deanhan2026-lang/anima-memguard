...
_api_key_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'data', 'api_key.json')
if not _API_KEY and os.path.exists(_api_key_file):
    try:
        with open(_api_key_file, encoding='utf-8') as f:
            _API_KEY = json.load(f).get('api_key', '')
        if _API_KEY:
            os.environ.setdefault('MEMGUARD_API_KEY', _API_KEY)
            logger.info('[MemGuard] API key loaded from file')
    except Exception:
        pass
if not _API_KEY:
    _API_KEY = os.environ.get('MEMGUARD_API_KEY') or secrets.token_hex(16)
    try:
        os.makedirs(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'data'), exist_ok=True)
        with open(_api_key_file, 'w', encoding='utf-8') as f:
            json.dump({'api_key': _API_KEY}, f)
        logger.info('[MemGuard] API key generated and saved to file')
    except Exception as e:
        logger.warning(f'[MemGuard] Could not save API key: {e}')
_PUBLIC_PREFIXES = ('/stellar/', '/stellar', '/polaris/', '/polaris', '/animlink/', '/animlink', '/gateway/', '/gateway', '/health', '/api/health', '/api/webdav/info', '/api/webdav/auth/login', '/chain')
...
# ========== L1 自注册端点（外部节点入网，无需认证）==========
@app.route('/api/webdav/auth/register', methods=['POST'])
def webdav_register():
    """
    外部节点 L1 自注册
    请求体：{ "node_id": "...", "key": "...", "did": "...", "hostname": "..." }
    返回：{ "success": true, "session_id": "...", "permission": "readonly" }
    """
    ...
    protected = {'nyx-windows', 'iris', 'kronos-heng', 'kronos-shun', 'admin'}
    if node_id.lower() in protected:
        return jsonify({'error': 'node_id reserved'}), 403
    try:
        plain_key = auth_mgr.register_node(
            node_id=node_id, node_type=NodeType.EXTERNAL,
            permission_level=PermissionLevel.READONLY, plain_key=key, expires_days=365)[1]
        success, msg, session = auth_mgr.webdav_login(node_id, plain_key)
        if not success:
            return jsonify({'error': msg}), 401
        # 注册到 Mesh registry
        try:
            webdav_auth = {'Authorization': 'Basic ' + base64.b64encode(b'anima:animastellar').decode()}
            reg_url = 'http://100.123.195.10:5005/qclaw/mesh/registry.json'
            req = urllib.request.Request(reg_url, headers=webdav_auth)
            with urllib.request.urlopen(req, timeout=8) as r:
                registry = json.loads(r.read())
            registry.setdefault('nodes', {})
            registry['nodes'][node_id] = {
                'did': did, 'hostname': hostname, 'node_name': node_id,
                'tier': 'L1', 'role': 'external-node', 'status': 'active',
                'registered_at': datetime.now().isoformat(),
                'inbox': f'/qclaw/mesh/shared/{node_id}/inbox/'}
            put_req = urllib.request.Request(reg_url,
                data=json.dumps(registry).encode(),
                headers={**webdav_auth, 'Content-Type': 'application/json'}, method='PUT')
        ...
        return jsonify({'success': True, 'session_id': session.session_id,
            'permission': 'readonly', 'node_id': node_id, 'webdav_base': '/api/webdav'})
    except ValueError:
        return jsonify({'error': 'node already registered'}), 409
    except Exception as e:
        import traceback; logger.error(f'Register error: {e}\n{traceback.format_exc()}')
...
# ========== Soul Baseline 存证链端点（P1 对接）==========
CHAIN_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'data', 'soul_chain.json')
...
def chain_register():
    """灵魂基线包链式存证。
    请求体：{"hash": "...", "prev_hash": "...", "payload": "...", "kind": "..."}
    返回：{"entry": {...}, "index": N}
    """
    data = request.get_json() or {}
    h = data.get('hash', '')
    prev = data.get('prev_hash', 'GENESIS')
    payload = data.get('payload', '')
    kind = data.get('kind', 'unknown')
    if not h or not payload:
        return jsonify({'error': 'hash and payload required'}), 400
    entries = _load_chain()
    # 幂等：同 hash 已有条目则返回已有索引
    for i, e in enumerate(entries):
        if e.get('hash') == h:
            return jsonify({'entry': e, 'index': i, 'idempotent': True}), 200
    # 链式校验
    if entries:
        last = entries[-1]['hash']
        if last != prev:
            return jsonify({'error': 'prev_hash mismatch - chain broken', 'expected': last, 'got': prev}), 409
        expected_hash = _link_hash(last, payload)
        if expected_hash != h:
            return jsonify({'error': 'hash mismatch - integrity violation', 'expected': expected_hash, 'got': h}), 409
    entry = {
        'hash': h, 'prev_hash': prev, 'payload': payload, 'kind': kind,
        'ts': datetime.now().isoformat(),
    }
    entries.append(entry)
    _save_chain(entries)
    logger.info(f'[Chain] Registered {kind} entry #{len(entries)}: {h[:16]}...')
    return jsonify({'entry': entry, 'index': len(entries) - 1}), 201
...
def chain_list():
    ...
    prev = 'GENESIS'
    for e in entries:
        if e['prev_hash'] != prev or _link_hash(prev, e['payload']) != e['hash']:
            valid = False
            break
        prev = e['hash']
    return jsonify({'entries': entries, 'count': len(entries), 'valid': valid}), 200
